...
public class SqliteCheckedOutItemDAO implements CheckedOutItemDAO {
    ...
    public List<CheckedOutItem> findByCheckOutId(int checkOutId) {
        ...
        try(PreparedStatement ps = ConnectionFactory.getPreparedStatement(sql)){
            ...
            while(rs.next()){
                ...
                if (receiver_id != 0) checkedOutItem.setReceiver(userDAO.findOne(receiver_id).orElseThrow());
    ...
    public List<CheckedOutItem> getReportByPart(int patrimonialId, LocalDateTime start, LocalDateTime end) {
        ...
        String sql = "SELECT * \n" +
                "FROM (checkout_item ci JOIN checkout ci.checkout_id ON c.checkout_id ) \n" +
                "WHERE ci. datetime(c.checkout_date) BETWEEN datetime(?) AND datetime(?);";
        try(PreparedStatement ps = ConnectionFactory.getPreparedStatement(sql)){
    ...
    public void update(CheckedOutItem checkedOutItem) {
        String sql ="UPDATE checkout_item \n" +
                "SET  return_date = ?, receiver_id = ?\n" +
                "WHERE checkout_id = ? AND part_item_id = ?";
        try(PreparedStatement ps = ConnectionFactory.getPreparedStatement(sql)){
            ps.setString(1, checkedOutItem.getReturnDate().toString());
            ps.setInt(2, checkedOutItem.getReceiver().getInstitutionalId());
            ps.setInt(3, checkedOutItem.getRelatedCheckout().getCheckOutId());
            ps.setInt(4, checkedOutItem.getPartItem().getPatrimonialId());
            ps.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
        }
